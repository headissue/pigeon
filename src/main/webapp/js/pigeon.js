...
;
  ...
  PigeonSurvey = (function() {
    ...
    PigeonSurvey.prototype.loadData = function() {
      ...
      Log.debug("load data");
      return $.getJSON(Pigeon.BASE_URL + 'api/survey/' + this.id + (Pigeon.useJSONP() ? '?callback=?' : ''), (function(_this) {
    ...
    PigeonSurvey.prototype.nextClicked = function(event) {
      var $el, el, lastAnswerEl, name, question, questionId, wrapper, _base, _self;
      ...
      if (question.type !== "multiple" && question.type !== "multiplefree") {
        ...
        if ((_base = this.answers)[questionId] == null) {
          _base[questionId] = [];
        }
      }
    ...
    PigeonSurvey.prototype.answerClicked = function(event) {
      var answer, el, name, question, questionId, _base;
      el = event.target || event.srcElement;
